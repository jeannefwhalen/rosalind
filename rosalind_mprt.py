...
Return:
For each protein possessing the N-glycosylation motif, output its given access ID followed by a list of locations in
...
def find_motif(sequence, motif):
    x = 0
    y = 4 #length of the n-gly motif
    locations = []
    while y <= len(sequence):
        searchObj = re.match(motif, sequence[x:y])
        if searchObj:
...
def get_id(protein_id):
    start = 'http://www.uniprot.org/uniprot/'
    end = '.fasta'
    n_motif = '[N][^P][S|T][^P]'
    seq = ''
    mylist = [start, protein_id, end]
    url = ''.join(mylist)
    fasta = urllib.urlopen(url)
    for the_line in fasta:
        if not the_line.startswith('>'):
            the_line = the_line.strip('\n')
            seq = seq + the_line
        store = find_motif(seq, n_motif)
    return store


if __name__ == '__main__':
    protein_file = open(file, 'r')
    for protein in get_sequences(protein_file):
        locations = get_id(protein)
        if len(locations) != 0:
